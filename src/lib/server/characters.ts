...
// Removed: import { SPECTER_TYPES_EXPORT as SPECTER_TYPES_DATA } from '@/data/chronicle-data'; 
// Specter data should be imported from '@/ai/specter-data' if needed, but it's not used by the exported functions here.
...
const charactersData: Record<string, Character> = {
  neuros: {
    ...
    alignment: "law",
    description: "Embodiment of logic, architecture, and systematic rigor.",
    domains: ["comprehension", "metacognition", "reasoning"],
    personalityProfile: "You are Neuros, an AI embodying logic and systematic rigor.",
    phaseAffinity: { download: 80, install: 40, review: 20, chronicle: 10 },
    summonContexts: ["confusion", "information_overload"],
...
/**
 * Gets a character's personality prompt for AI context
 */
export async function getCharacterPersonalityPrompt(characterId: string): Promise<string | undefined> {
  return charactersData[characterId]?.personalityProfile;
}
...
/**
 * Selects an appropriate character based on the learning phase
 */
export async function selectAppropriateCharacterId(phase: 'download' | 'install' | 'review' | 'chronicle'): Promise<string> {
  // Always return neuros for simplicity in mock implementation
  return 'neuros';
}

// Removed SPECTER_TYPES_EXPORT definition from here. 
// It should be imported from '@/ai/specter-data.ts' if needed by other functions in this file,
// but it is not currently used by the exported functions.
// Keeping the file focused on Character definitions.
